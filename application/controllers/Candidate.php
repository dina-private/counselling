...
class Candidate extends Controller
{
    ...
    {
        $stateObject = $this->loadModel('StateModel');
        $allState = $stateObject->getAll();

        $this->loadView('management/manage-candidates')
            ->setData('states', $allState)
            ->render();
    }

    public function ajaxHandler()
    {
        $candidateObject = $this->loadModel('CandidateModel');
        ...
        switch ($data['method']) {
            case 'getCandidates':
                ...
                $orgList = $candidateObject->getAll();
